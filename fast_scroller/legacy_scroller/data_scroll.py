...
def volumetric_data(data, rowcol):
    """Utility to prepare volumetric data for VTK without resorting to mem copying (if possible)"""
...
class DataScroller(HasTraits):
    ...
    ## array scene (Mayavi components, utilized if Mayavi is importable)
    _has_video = Bool
    array_scene = Any
    ...
    def default_traits_view(self):
        ...
        if use_mayavi:
            ...
            top_group = HGroup(
                Item(
                    'zoom_plot', editor=tb.MPLFigureEditor(),
                    show_label=False, width=700, height=200, resizable=True
                )
            )
